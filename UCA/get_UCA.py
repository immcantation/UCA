...
# right now input params are 
# 0: get_UCA.py -- script name
# 1: clone_ids -- clone ids to get UCA for
# 2: directory -- directory where the clone data is stored
# 3: max iters -- max number of iterations to run
# 4: nproc -- number of processors to use
# 5: id -- the name of the folder thats created to store the data
# 6: model_folder -- the folder where the olga model information is stored
# 7: quiet -- whether to print out the progress/messages of the script
...
def process_option(option, filtered_df, codon, current_germline, starting_point, ending_point, igphyml_df_new):
    ...
    while still_improving and iteration < max_iter:
        ...
        for codon in codon_list:           
            ...
            with Pool(nproc) as pool:
                results = pool.map(process_option_wrapper, [(option, filtered_df, codon, current_germline, starting_point, ending_point, igphyml_df_new) for option in range(0, len(filtered_df))])
